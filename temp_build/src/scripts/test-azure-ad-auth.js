...
async function testAzureADAuth() {
    ...
    try {
        ...
        const isCLIAvailable = await azure_ad_auth_helper_1.AzureADAuthHelper.isAzureCLIAvailable();
        if (!isCLIAvailable) {
        ...
        const isLoggedIn = await azure_ad_auth_helper_1.AzureADAuthHelper.isUserLoggedIn();
        if (!isLoggedIn) {
        ...
        const accessToken = await azure_ad_auth_helper_1.AzureADAuthHelper.getAccessToken();
        if (!accessToken) {
        ...
        try {
            const options = await (0, data_source_1.createDataSourceOptions)();
...
//# sourceMappingURL=test-azure-ad-auth.js.map