...
async function registerColombiaticTenant() {
  try {
    ...
    // Registrar como tenant propietario usando el endpoint correcto
    const response = await axios.post(
      'http://localhost:3007/api/meta-agent/tenants/owner/register',
      ownerTenantData,
      {
        headers: {
    ...
    // Registrar los servicios del tenant usando el endpoint correcto
    const servicesData = {
      services: [
        {
          id: "desarrollo-web",
    ...
    // Esperar un momento para que el tenant se cree completamente
    await new Promise(resolve => setTimeout(resolve, 2000));
    
    // Actualizar los productos y servicios del tenant
    const servicesResponse = await axios.put(
      'http://localhost:3007/api/meta-agent/tenants/colombiatic',
      servicesData,
      {
        headers: {
          'Content-Type': 'application/json'
