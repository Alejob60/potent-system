...
function logAgentStatus(message, agent, status, error) {
    ...
    if (status === 'OK') {
        ...
        console.error(`  [${agent}] ${message}${error ? `: ${error instanceof Error ? error.message : error}` : ''}`);
    }
}
async function testAgentFaqResponder() {
    const baseUrl = process.env.AGENT_FAQ_RESPONDER_URL ||
...
//# sourceMappingURL=agent-faq-responder.check.js.map