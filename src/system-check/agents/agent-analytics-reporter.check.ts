...
async function testAgentAnalyticsReporter() {
  const baseUrl = process.env.AGENT_ANALYTICS_REPORTER_URL || 'http://localhost:3000/agents/analytics-reporter';

  try {
    // Test POST
    const createRes = await axios.post(baseUrl, { metric: 'engagement', period: 'daily' });
    if (!createRes.data || !createRes.data.id) throw new Error('POST respuesta inválida');
    logAgentStatus('POST Ok', 'AnalyticsReporter', 'OK');

    // Test GET
    const listRes = await axios.get(baseUrl);
    if (!Array.isArray(listRes.data)) throw new Error('GET all respuesta inválida');
    logAgentStatus('GET All Ok', 'AnalyticsReporter', 'OK');

    // Test GET/:id
    const id = createRes.data.id;
    const getRes = await axios.get(`${baseUrl}/${id}`);
    if (!getRes.data || getRes.data.id !== id) throw new Error('GET:id respuesta inválida');
    logAgentStatus('GET by Id Ok', 'AnalyticsReporter', 'OK');

  } catch (error) {
    logAgentStatus('Error de prueba', 'AnalyticsReporter', 'FAIL', error);
  }
}

if (require.main === module) {
  testAgentAnalyticsReporter();
}
