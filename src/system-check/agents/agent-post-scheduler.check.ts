...
async function testAgentPostScheduler() {
  const baseUrl = process.env.AGENT_POST_SCHEDULER_URL || 'http://localhost:3000/agents/post-scheduler';

  try {
    // Test POST
    const scheduledAt = new Date(Date.now() + 60 * 1000).toISOString();
    const createRes = await axios.post(baseUrl, { content: 'demo post', scheduledAt });
