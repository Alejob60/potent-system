...
async function checkAndDropTable() {
  try {
    ...
    // Check if there are any records in the migrations table for this migration
    try {
      ...
      if (migrationResult.rows.length > 0) {
        console.log('Migration already recorded in migrations table');
        return;
      }
    } catch (e) {
    ...
    console.error('Error checking/dropping table:', error.message);
  } finally {
    await client.end();
  }
}
